    ...
    return (
      <div className="space-y-3">
        {Object.entries(documents).map(([key, doc]) => (
          doc && doc.fileName ? (
            <div key={key} className="flex items-center justify-between p-4 bg-gray-50 border border-gray-200 rounded-xl hover:shadow-sm transition-all">
              <div className="flex items-center flex-1 min-w-0">
                <div className="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center mr-3 text-blue-600 flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    {doc.compressed && (
                      <>
                        <p className="text-xs text-gray-400">•</p>
                        <span className="text-xs bg-green-100 text-green-700 px-1 rounded">Optimized</span>
                      </>
                    )}
                  </div>
                  {doc.extractedText === '' && (
              ...
              <div className="flex items-center gap-2 ml-3">
                <button
                  ...
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-colors duration-200 flex items-center gap-1"
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-medium rounded-lg transition-colors duration-200 flex items-center gap-1"
                  title="View document"
                >
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  // ...existing code...
    ...
    async function uploadPDFBlob(report, userId, pdfBlob) {
      try {
        ...
        const { data: mainUrlData, error: mainUrlError } = supabase.storage
          .from('user_analysis_report')
          .getPublicUrl(fileName);
        
        if (mainUrlError || !mainUrlData?.publicUrl) {
          console.log('⚠️ Main bucket URL failed, trying alternative bucket');
          const { data: altUrlData, error: altUrlError } = supabase.storage
            .from('reports')
            .getPublicUrl(fileName);
          
          if (altUrlError || !altUrlData?.publicUrl) {
            console.error('❌ Failed to get public URL from both buckets:', { mainUrlError, altUrlError });
            return null;
          }
          urlData = altUrlData;
        } else {
    ...
    return (
      <div className="min-h-screen bg-gray-50/50 font-sans text-gray-800 pb-12" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
        <div className="max-w-[1600px] mx-auto p-4 sm:p-6 lg:p-8 lg:ml-16">
            ...
            {/* Main Content Grid */}
            <div className={`grid grid-cols-1 gap-6 items-start ${userDetails.role === 'advisor' || userDetails.role === 'admin' ? 'lg:grid-cols-1' : 'lg:grid-cols-12'}`}>
                
                {/* Left Column: Profile & Documents */}
                <div className={`space-y-6 ${userDetails.role === 'advisor' || userDetails.role === 'admin' ? 'max-w-4xl mx-auto' : 'lg:col-span-5'}`}>
                    {/* Profile Card */}
                    <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm relative overflow-hidden max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                                    ...
                                    <div className="flex items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-sm font-medium text-gray-900">
                                              {(() => {
                                                const countries = profileData?.target_countries || profileData?.country || [];
                    ...
                    {/* Documents Section */}
                    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 min-h-[500px] flex flex-col">
                      ...
                      <div className="border-t border-gray-200 pt-4 flex-1 overflow-y-auto">
                ...
                {/* Right Column: Report & Analysis (7 cols) - Only for Students */}
                {(userDetails.role === 'student' || userDetails.role === 'user') && (
                        ...
                        <button 
                          onClick={async () => {
                            try {
                              ...
                              let url = latestReport?.url || reportUrl;
                              
                              console.log('📋 Attempting to download report:', { latestReport, reportUrl, url });
              ...
              if (!url && profileData?.application_report) {
                ...
                const newUrl = await uploadAndSavePDF(user.id);
                if (newUrl) {
                  url = newUrl;
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 p-6">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="p-4 bg-green-50 rounded-xl border border-green-100 text-center">
                          ...
                          <p className="text-2xl font-bold text-green-700">{profileData?.budget ? '$' + (profileData.budget / 1000).toFixed(1) + 'k' : 'No data'}</p>
                        </div>
                        ...
                        <table className="w-full border-collapse">
                          ...
                          <tbody>
                            ...
                            <tr className="border-b border-gray-100">
                              ...
                              <td className="p-3 text-sm text-blue-600">
                                {profileData?.budget ? `Your budget ($${(profileData.budget / 1000).toFixed(1)}k) covers basic tuition. Consider additional living costs.` : 'Your budget information is not available. Please complete your profile.'}
                              </td>
                            </tr>
                            <tr className="border-b border-gray-100">
                              <td className="p-3">
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 p-6">
                      ...
                      <div className="space-y-3">
                        {(() => {
                          const countries = (profileData?.country && Array.isArray(profileData.country) && profileData.country.length > 0 
                            ? profileData.country 
                            : (profileData?.target_countries && Array.isArray(profileData.target_countries) ? profileData.target_countries : []));
                          
                          // Check if user has complete profile data for university matching
                          ...
                          // Profile is complete - check for actual university matches
                          // TODO: Replace with actual university matching API call
                          const universityMatches = []; // This would come from your university matching service
                          
                          if (universityMatches.length === 0) {
                            // No matches found despite complete profile
                            return (
                              <div className="text-center py-8">
                                <div className="text-gray-400 mb-4">
                                  <svg className="w-16 h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                  </svg>
                                </div>
                                ...
                                <div className="bg-blue-50 rounded-lg p-4 mb-4">
                                  <h5 className="font-semibold text-blue-800 mb-2">Your Current Profile:</h5>
                                  <div className="text-sm text-blue-700 space-y-1">
                                    <div>• GPA: {profileData?.gpa}</div>
                                    <div>• Language Score: {profileData?.language_score}</div>
                                    <div>• Budget: ${(profileData?.budget / 1000).toFixed(1)}k</div>
                                    <div>• Target Countries: {countries.join(', ')}</div>
                                  </div>
                                </div>
                                <div className="space-y-3">
                                  <h5 className="font-semibold text-gray-700">Suggestions to find matches:</h5>
                                  <div className="text-sm text-gray-600 space-y-2">
                                    <div>• Consider expanding your target countries</div>
                                    <div>• Increase your budget range if possible</div>
                                    <div>• Look into programs with lower GPA requirements</div>
                                    <div>• Consider foundation or pathway programs</div>
                                  </div>
                                </div>
                          ...
                          // Show university matches if found
                          return universityMatches.map((university, index) => (
                            <div key={index} className="flex items-center justify-between p-4 rounded-xl border bg-green-50 border-green-200">
                              <div className="flex items-center gap-4">
                                <span className="font-bold text-lg text-[#1a0841] w-6">{index + 1}.</span>
                                <div>
                                  <span className="font-semibold text-[#1a0841] block">{university.name}</span>
                              ...
                              <span className="text-2xl font-bold text-[#6c47ff]">{university.matchPercentage}%</span>
                            </div>
                          ));
                        })()}
                      </div>
                    ...
                    {/* AI Button Section - Only for Students */}
                    <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', marginTop: '1rem' }}>
                      <button
                        onClick={handleGetAIAdvice}
                        style={{ background: '#a020f0', color: 'white', border: 'none', padding: '0.7rem 1.5rem', borderRadius: '999px', cursor: 'pointer', fontWeight: 'bold' }}
                      >
                        Get AI Advice
                      </button>
                    </div>
            ...
            {/* Loading and Update Messages */}
            {updateError && <div className="text-red-600 text-xs mt-4 text-center bg-red-50 rounded px-4 py-2 border border-red-200">{updateError}</div>}
            {updateSuccess && <div className="text-green-600 text-xs mt-4 text-center bg-green-50 rounded px-4 py-2 border border-green-200">{updateSuccess}</div>}
        </div>
      </div>
    );
  }
